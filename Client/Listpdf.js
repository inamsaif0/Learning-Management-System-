...
export default function Listpdf() {
  ...
  async function handleFilter(specifiedDate) {
    try {
      const response = await fetch(`http://3.35.175.207:3000/documents?email=${email.userEmail}`, {
        method: 'GET'
      });
  ...
  async function getDocs(email) {
    try {
      const response = await fetch(`http://3.35.175.207:3000/documents?email=${email}`, { method: 'GET' })
